    ...
    B3State evaluateNode(B3Node node) {
      ...
      if (node.duration != null && node.duration! < scenario.duration) {
        ...
        traces[node.id]!.add(TraceStep(
            ...
            description: 'Resource duration insufficient'));
      ...
      if (node.children.isEmpty) {
        ...
        if (node.duration == null) {
          traces[node.id]!.add(TraceStep(
              ...
              description: 'No children dependencies'));
        }
        visiting.remove(node.id);
        return baseState;
      }
      ...
      if (node is Capability) {
        ...
        if (state == B3State.failed || state == B3State.degraded) {
          result.vulnerabilities
              .add(Vulnerability(node, state, result.getRootCauses(node.id)));
        } else if (state == B3State.unknown || state == B3State.notAssessed) {
          result.uncertainties
              .add(Uncertainty(node, state, result.getRootCauses(node.id)));
        }
      }
    }
