...
void main() {
  ...
  test('Test : Foyer partiellement diagnostiqué (NOT_ASSESSED)', () {
    ...
    expect(
        result.vulnerabilities.any((v) => v.capability.id == 'boire'), false);
    expect(
        result.uncertainties.any((u) =>
            u.capability.id == 'boire' && u.state == B3State.notAssessed),
        true);
  });
  ...
  test('Test : Je ne sais pas (UNKNOWN)', () {
    ...
    expect(
        result.uncertainties.any(
            (u) => u.capability.id == 'cuisiner' && u.state == B3State.unknown),
        true);
  });
  ...
  test('Test : Évolution du diagnostic', () {
    ...
    expect(result.nodeStates['eclairage'],
        B3State.failed); // Devient une absence confirmée
  });
