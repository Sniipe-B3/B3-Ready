...
void main() {
  ...
  test(
      'PARCOURS 1 : Le diagnostic se comporte comme une conversation (Foyer A - Résilient)',
      () {
    ...
    state.answerQuestion(
        currentQuestion!.id, 'opt_yes_rechaud'); // Réponse : Oui, réchaud gaz

    // Q4 : Chauffage
    currentQuestion = diagEngine.getNextQuestion(state);
    expect(currentQuestion?.id, 'q_heat_main');
    state.answerQuestion(
        currentQuestion!.id, 'opt_poele'); // Réponse : Poêle à bois

    // Q5 : Éclairage
    currentQuestion = diagEngine.getNextQuestion(state);
    expect(currentQuestion?.id, 'q_light_main');
    state.answerQuestion(
        currentQuestion!.id, 'opt_lamp_bat'); // Réponse : Lampe sur batterie
    ...
    expect(result.nodeStates['cuisiner'],
        B3State.maintained); // Maintenu par le réchaud gaz
    expect(result.nodeStates['chauffer'],
        B3State.maintained); // Maintenu par le poêle à bois
    expect(result.nodeStates['eclairage'],
        B3State.maintained); // Maintenu par la lampe batterie

    expect(
        result.vulnerabilities.any((v) =>
            ['cuisiner', 'chauffer', 'eclairage'].contains(v.capability.id)),
        false);
  });
  ...
  test('PARCOURS 4 : Incertitude protégée (Foyer C)', () {
    ...
    final scen = Scenario(
        name: 'Panne',
        duration: Duration(),
        systemOverrides: {'unk_sys': B3State.unknown});
    ...
    expect(result.vulnerabilities.any((v) => v.capability.id == 'cuisiner'),
        false);
    // C'est une incertitude (qui nécessite un diagnostic plus poussé)
    expect(
        result.uncertainties.any((u) => u.capability.id == 'cuisiner'), true);
  });
