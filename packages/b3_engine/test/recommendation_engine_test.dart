...
void main() {
  ...
  test('TEST 1 — Vulnérabilité simple', () {
    ...
    expect(
        recs.any((r) =>
            r.capabilityId == 'cuisiner' && r.targetAssetId == 'rechaud_gaz'),
        true);
  });
  ...
  test('TEST 4 & 5 — Incertitude et NOT_ASSESSED', () {
    ...
    expect(
        recs.any((r) => r.type == RecommendationType.createAlternative), false);
  });
  ...
  test('TEST 6 — Explication (Traceability)', () {
    ...
    final result = B3Engine().runSimulation(
        DataMapper.buildGraph(b3KnowledgeBase, household), scenario);
    final recs = recEngine.generate(result, household, scenario);
