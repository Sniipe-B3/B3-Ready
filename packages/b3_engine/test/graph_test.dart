...
void main() {
  ...
  test('Step 4 & 5: First Graph and ReasoningTrace', () {
    ...
    final plaque =
        Asset(id: 'plaque', name: 'Plaque électrique', children: [electricite]);
    final cartouche = Resource(id: 'cartouche', name: 'Cartouche gaz');
    final rechaud =
        Asset(id: 'rechaud', name: 'Réchaud gaz', children: [cartouche]);
  ...
  test('Step 6: DEGRADED Case (Resource duration)', () {
    final wood =
        Resource(id: 'wood', name: 'Bois', duration: Duration(hours: 24));
    final stove = Asset(id: 'stove', name: 'Poêle à bois', children: [wood]);
    final heating =
        Capability(id: 'heating', name: 'Chauffage', children: [stove]);

    final result = engine.runSimulation([
      wood,
      stove,
      heating
    ], Scenario(name: 'S', duration: Duration(hours: 72), systemOverrides: {}));
  ...
  test('Step 7: UNKNOWN Cases in ANY logic', () {
    ...
    final unknownNode =
        System(id: 'unk', name: 'Unk', overriddenState: B3State.unknown);
    final mainNode =
        System(id: 'main', name: 'Main', overriddenState: B3State.maintained);
    final degNode =
        System(id: 'deg', name: 'Deg', overriddenState: B3State.degraded);

    final anyFailUnk =
        Capability(id: 'cap1', name: 'Cap1', children: [failNode, unknownNode]);
    expect(
        engine.runSimulation(
            [failNode, unknownNode, anyFailUnk],
            Scenario(
                name: 'S',
                duration: Duration(),
                systemOverrides: {})).nodeStates['cap1'],
        B3State.unknown);

    final anyMainUnk =
        Capability(id: 'cap2', name: 'Cap2', children: [mainNode, unknownNode]);
    expect(
        engine.runSimulation(
            [mainNode, unknownNode, anyMainUnk],
            Scenario(
                name: 'S',
                duration: Duration(),
                systemOverrides: {})).nodeStates['cap2'],
        B3State.maintained);

    final anyDegUnk =
        Capability(id: 'cap3', name: 'Cap3', children: [degNode, unknownNode]);
    expect(
        engine.runSimulation(
            [degNode, unknownNode, anyDegUnk],
            Scenario(
                name: 'S',
                duration: Duration(),
                systemOverrides: {})).nodeStates['cap3'],
        B3State.degraded);
  });
  ...
  test('Test A: Cycle Detection', () {
    ...
    final result = engine.runSimulation([nodeA, nodeB],
    ...
    expect(
        result.traces['a']!.steps.any((s) =>
            s.type == ReasonType.cycleDetected ||
            s.type == ReasonType.evaluatedChildren),
        true);
  });
  ...
  test('Test B: Structured ReasoningTrace', () {
    ...
    final scenario = Scenario(
        ...
        systemOverrides: {'elec': B3State.failed});
    final result =
        engine.runSimulation([elec, plaque, gaz, rechaud, cuisiner], scenario);
  ...
  test('Test C: UNKNOWN vs Vulnerability', () {
    ...
    final result = engine.runSimulation(
        [unkSys, failSys, capUnknown, capFailed],
        Scenario(name: 'S', duration: Duration(), systemOverrides: {}));
  ...
  test('Test D: UNKNOWN protected', () {
    ...
    final cap = Capability(
        id: 'cap', name: 'Cap', children: [unkSys, mainSys]); // rule = ANY

    final result = engine.runSimulation([unkSys, mainSys, cap],
        Scenario(name: 'S', duration: Duration(), systemOverrides: {}));
  ...
  test('Test E: Determinism', () {
    ...
    expect(
        r1.traces['cap']!.steps.last.state, r2.traces['cap']!.steps.last.state);
  });
}
